	...
	extends Hangul
{
	public static final String[] COND_ARR = { 
			...
			"가다", 		// 27 '가'로 끝나는 동사
			"오다", 		// 28 '오'로 끝나는 동사
			"ENG", 		// 29 영문을 소리나는 대로 읽은 말
			"체언", 		// 30 체언으로 사용됨
			"관형어", 	// 31 관형어로 사용됨
			"부사어", 	// 32 부사어로 사용됨
			"서술어", 	// 33 서술어로 사용됨
			"EC", 		// 34 보조적 연결 어미로 사용됨.	~고 있~, ~지 않~ 과 같이 특정하게 나오는 패턴을 위해서 보조적 연결어미 설정
			"F", 		// 35 보조사 '는'과 같이 어절의 끝에 오는 형태소를 위해 사용
			"생략" 		// 36 바다(이)다와 같이 (이)가 가운데 생략된 것을 표현하기 위한 기분석 후보를 표현, 생략된 것을 위한 기분석 후보는 띄어쓰기 될 수 없음.
		};
