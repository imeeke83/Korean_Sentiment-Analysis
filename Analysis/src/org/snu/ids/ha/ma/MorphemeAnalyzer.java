 ...
 * 띄어쓰기 단위로 수행하지 않고, token 단위로 동적 프로그래밍을 하여,
...
public class MorphemeAnalyzer
{
		...
		throws Exception
	{
		...
		for( ; tailCutPos <= strlen; tailCutPos++ ) {
			...
			if( !meCur.isComplete() && tailCutPos < 11  ) continue;
		...
		throws Exception
	{
		...
		List<MExpression> tempMEList = new ArrayList<MExpression>();
		...
		throws Exception
	{
		...
		for( int i = 0; i < melAnalResult.size(); i++ ) {
			...
			{
				// canceled addAll로 처리함으로써 기호를 lastMorp로 처리하지 않음.
				// eojeol.addAll(me1.get(0));
				eojeol.add(me1.get(0).firstMorp);
				eojeol.exp += me1.exp;
			} else {
