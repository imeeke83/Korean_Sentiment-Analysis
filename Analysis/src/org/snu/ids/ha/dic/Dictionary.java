...
public class Dictionary
{
	...
	public boolean containNoun(String exp)
	{
		...
		for( MCandidate mc : me ) {
			if( mc.size() == 1 && mc.isFirstTagOf(POSTag.NNA) ) return true;
		}
		return false;
	}
		...
		throws Exception
	{
		...
		try {
			...
			while( (line = simpleDicReader.readLine()) != null ) {
				...
				String atl = null, hcl = null, ccl = null, ecl = null, compResult = null;
				if( condInfo != null ) {
					// 부가 정보들에 대한 처리 수행
...
//					}
	...
	public List<Morpheme> getWordList()
	{
		...
		for( MExpression me : table.values() ) {
			for( MCandidate mc : me ) {
				if( mc.size() == 1 ) {
